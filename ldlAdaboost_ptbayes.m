function [resultSingle,resultAll] = ldlAdaboost_ptbayes(trainFeatures,trainLabels,testFeatures,testLabels,boostTimes,nboot,factor)
if nargin == 4
    boostTimes = 5;
    nboot = 1;
    factor = 1.0;
...
% Adaboosting
    ...
    for i = 1:boostTimes 
        ...
        model{i} = ptbayesTrain(bootTrainFeatures{i},bootTrainLabels{i});
        preTrainLabels{i} = ptbayesPredict(model{i},trainFeatures);
    ...
    for n = 1:boostTimes
        ...
        for i = 1:n 
            preLabels{i} = ptbayesPredict(model{i},testFeatures);
