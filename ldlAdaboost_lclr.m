function [resultSingle,resultAll,preLabels] = ldlAdaboost_lclr(trainFeatures,trainLabels,testFeatures,testLabels,boostTimes,nboot,factor)
if nargin == 4
    boostTimes = 5;
    nboot = 1;
    factor = 1.0;
...
% Adaboosting
    ...
    for i = 1:boostTimes 
        ...
        if i == 1
            [weights{i},~] = lclrLdlTrain(item,bootTrainFeatures{i},bootTrainLabels{i});
        else
            [weights{i},~] = lclrLdlTrain(weights{i-1},bootTrainFeatures{i},bootTrainLabels{i});
        end
    ...
    for n = 1:boostTimes
        ...
        for i = 1:n 
            preLabels{i} = bfgsPredict(weights{i},testFeatures);
