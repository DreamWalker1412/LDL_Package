function [weights,fval,exitFlag,output,grad] = kqaLdlTrain_raw(xInit,trainFeatures,trainLabels,para,optim)
...
if (~exist('optim','var')) 
 ...
 %  [weights,fval,exitFlag,output,grad] = fminlbfgs(@bfgsProcess,xInit);
    ...
    elseif  method == -1
        QA = log(1+kurtosis(trainLabels,1,2));
    elseif method == -2
        QA = ones(size(kurtosis(trainLabels,1,2)));
    else
