function [resultSingle,resultAll,preLabels] = ldlAdaboost_cpnn(trainFeatures,trainLabels,testFeatures,testLabels,boostTimes,nboot,factor)
if nargin == 4
    boostTimes = 5;
    nboot = 1;
    factor = 1.0;
...
% Adaboosting
    ...
    for i = 1:boostTimes 
        ...
        model{i} = cpnnTrain(bootTrainFeatures{i},bootTrainLabels{i});
        preTrainLabels{i} = cpnnPredict(model{i},trainFeatures);
    ...
    for n = 1:boostTimes
        ...
        for i = 1:n 
            preLabels{i} = cpnnPredict(model{i},testFeatures);
