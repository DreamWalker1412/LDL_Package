function [weights,fval,exitFlag,output,grad] = ldlTrain(xInit,trainFeatures,trainLabels,optim)
%BFGSLLDTRAIN	The training part of BFGSLDL algorithm.
%
%	Description
...
if (~exist('optim','var')) 
  ...
  %  [weights,fval,exitFlag,output,grad] = fminlbfgs(@bfgsProcess,xInit);
    ...
    delta=1e-10;
    [row,col]=size(modProb);
    for i=1:row
        for j=1:col
            if isnan(modProb(i,j))
                modProb(i,j)=delta;
            end
        end
    end
    ...
    % Target function.
    target = -sum(sum(trainLabels.*log(modProb+delta)));

    % The gradient.
    gradient = trainFeatures'*(modProb - trainLabels);
