...
defaultopt = struct('Display','iter','HessUpdate','bfgs','GoalsExactAchieve',1,'GradConstr',true,  ...
			'TolX',1e-5,'TolFun',1e-5,'GradObj','on','MaxIter',300,'MaxFunEvals',100*numel(xInit)-1,  ...
			'DiffMaxChange',1e-1,'DiffMinChange',1e-6,'OutputFcn',[], ...
...
if (~exist('optim','var')) 
  ...
  for i=1:length(f)
     if (~isfield(optim,f{i})||(isempty(optim.(f{i}))))
         optim.(f{i})=defaultopt.(f{i}); 
     end
  end
end
...
data.timeTotal=tic;
data.timeExtern=0;
% Switch to L-BFGS in case of more than 2000 unknown variables
if(optim.HessUpdate(1)=='b') 
    if(data.numberOfVariables<2000) 
...
if(strcmp(optim.Display,'iter'))
        s=sprintf('     %5.0f       %5.0f       %5.0f       %13.6g    ',data.iteration,data.funcCount,data.gradCount,data.fInitial); disp(s); %#ok<*DSPS>
end
